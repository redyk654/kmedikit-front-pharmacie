...
export default function ListingFactures(props) {
    ...
    useEffect(() => {
        ...
        if (dateDepart.length > 0 && dateFin.length > 0) {
            ...
            req.open('POST', `${nomDns}listing_pharmacie.php`);
            ...
            req.addEventListener('load', () => {
                ...
                if (filtre) {
                    result = result.filter(item => (item.vendeur.toLowerCase() == vendeur.toLowerCase()));
                }
    ...
    }, [dateDepart, dateFin, vendeur, assurance]);
    ...
    useEffect(() => {
        ...
        req.addEventListener('load', () => {
            if(req.status >= 200 && req.status < 400) {
                setMessageErreur('');
                let result = JSON.parse(req.responseText);
                result = result.filter(item => (item.rol === "vendeur"))
    ...
    const recupererRecetteTotal = (result) => {
            if (props.role.toLowerCase() === "vendeur") {
                result = result.filter(item => (item.vendeur.toLowerCase() == props.nomConnecte.toLowerCase()));
            } else {
                if (filtre) {
                    result = result.filter(item => (item.vendeur.toLowerCase() == vendeur.toLowerCase()));
                }
    ...
    const rechercherHistorique = () => {
        ...
        if (filtre) {
            setVendeur(document.getElementById('caissier').value.toLowerCase());
        } else {
            setVendeur('tout');
        }
    }
